...
### 2. Cấu hình ban đầu
Di chuyển vào thư mục dự án và thực hiện tải các thư viện cần thiết (Gson):
```bash
mvn clean install
```

### 3. Khởi chạy ứng dụng trực tiếp từ Terminal (Khuyên dùng)
Bạn có thể dọn dẹp, biên dịch lại toàn bộ và khởi chạy ứng dụng trực tiếp bằng một câu lệnh duy nhất mà không cần đóng gói:
```bash
